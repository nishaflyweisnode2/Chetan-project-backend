...
new cronJob('*/30 * * * * *', async function () {
  ...
  let findState = await Subscription.find({ cutOffOrderType: "eveningOrder", type: "EveryDay", firstTimeOrder: false, orderCreateTill: { $gte: moment().startOf('day').toDate(), $lte: moment().endOf('day').toDate() } }).populate([{ path: 'userId', populate: { path: "addressId" } }, { path: 'product' }]);
  if (findState.length > 0) {
    for (let i = 0; i < findState.length; i++) {
      const startDate = new Date(findState[i].startDate);
      const endDate = new Date(startDate);
...
new cronJob('* * * * * *', async function () {
  ...
  let findState = await Subscription.find({ cutOffOrderType: "morningOrder", type: "EveryDay", firstTimeOrder: true }).populate([{ path: 'userId', populate: { path: "addressId" } }, { path: 'product' }]);
  if (findState.length > 0) {
    for (let i = 0; i < findState.length; i++) {
...
new cronJob('* * * * * *', async function () {
  const currentDate = moment().utc();
  let findState = await Subscription.find({ cutOffOrderType: "eveningOrder", type: "EveryDay", firstTimeOrder: true }).populate([{ path: 'userId', populate: { path: "addressId" } }, { path: 'product' }]);
  if (findState.length > 0) {
    for (let i = 0; i < findState.length; i++) {
      const startDate = new Date(findState[i].orderCreateTill);
      startDate.setDate(startDate.getDate() + 1);
