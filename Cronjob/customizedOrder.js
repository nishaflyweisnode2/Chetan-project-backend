...
new cronJob('* * * * * *', async function () {
  ...
  let findState = await Subscription.find({ cutOffOrderType: "morningOrder", type: "customized", firstTimeOrder: true }).populate([{ path: 'userId', populate: { path: "addressId" } }, { path: 'product' }]);
  if (findState.length > 0) {
    for (let i = 0; i < findState.length; i++) {
...
new cronJob('* * * * * *', async function () {
  const currentDate = moment().utc();
  let findState = await Subscription.find({ cutOffOrderType: "eveningOrder", type: "customized", firstTimeOrder: true }).populate([{ path: 'userId', populate: { path: "addressId" } }, { path: 'product' }]);
  if (findState.length > 0) {
    for (let i = 0; i < findState.length; i++) {
      const startDate = new Date(findState[i].orderCreateTill);
      startDate.setDate(startDate.getDate() + 1);
